...
current_turn = 0
lock = threading.Lock()
symbols = ['X', 'O']  # Global symbols array for player symbols
...
def client_thread(conn, player):
    ...
    welcome_message = f"Welcome to Tic Tac Toe! You are Player {player + 1} ({player_symbol})"
            ...
            try:
                ...
                if player == current_turn:
                    ...
                    if game_board[x][y] == ' ':
                        ...
                        win, winner = check_win(game_board)
                        if win:
                            send_game_board_to_all()
                ...
                print(f"Error handling data from player {player + 1}: {e}")
                break
...
def send_game_board_to_all():
    with lock:
        ...
        send_to_all(board_visual + f"\nIt's Player {current_turn + 1}'s turn ({symbols[current_turn]}).")
